...
export function App() {
  ...
  return (
    <div className="flex h-screen min-h-0 items-center justify-center overflow-hidden bg-[#070A0F] p-5 text-slate-100">
      <main className="flex h-full w-full max-w-[560px] flex-col">
        <header className="flex items-center justify-between border-b border-white/10 pb-5">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-teal-200/15 bg-[#0A1220] shadow-lg shadow-black/25">
              <img src={appIconUrl} alt="" aria-hidden="true" className="h-10 w-10 rounded-lg" />
            </div>
            <div>
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.04] px-3 py-2 text-right tabular-nums">
            <div className="text-lg font-semibold leading-none text-white" aria-live="polite">
              {openCount}
            </div>
        ...
        <section className="flex min-h-0 flex-1 flex-col items-center justify-center py-6 text-center">
          <button
            type="button"
            aria-label="Launch Codex"
            aria-busy={pendingLaunches > 0}
            onClick={() => void launchCodex()}
            className="flex h-28 w-28 shrink-0 items-center justify-center rounded-full border border-teal-200/50 bg-teal-300 text-[#03110F] shadow-[0_18px_80px_rgba(20,184,166,0.24)] transition hover:scale-[1.03] hover:bg-teal-200 focus:outline-none focus:ring-4 focus:ring-teal-300/20 active:scale-[0.99]"
          >
            <Plus size={54} strokeWidth={2.35} />
          </button>

          <div className="mt-7 text-sm font-medium text-slate-300" aria-live="polite">
            {openWindowLabel}
          </div>
          {lastEvent ? <p className="mt-2 max-w-sm truncate text-xs text-slate-500">{lastEvent.message}</p> : null}
        </section>

        <section className="max-h-[38vh] overflow-hidden rounded-lg border border-white/10 bg-white/[0.035] text-left shadow-2xl shadow-black/20">
          ...
          <div className="max-h-[calc(38vh-46px)] overflow-y-auto">
            {openCount === 0 ? (
              ...
              snapshot.instances.map((instance) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-xs text-slate-500 tabular-nums">PID {instance.pid ?? "pending"}</div>
                  </div>
                  <span
